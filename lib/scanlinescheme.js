...
function ScanlineScheme(options) {
    ...
    for (var z = options.minzoom; z <= options.maxzoom; z++) {
        ...
        // Adjust include all tiles within the metatile in the stats
        if (z === 0) {
            this.stats.total++;
        } else {
            var minX = this.bounds[z].minX - (this.bounds[z].minX % this.metatile);
            var maxX = this.bounds[z].maxX - (this.bounds[z].maxX % this.metatile) + this.metatile - 1;
            var minY = this.bounds[z].minY - (this.bounds[z].minY % this.metatile);
            var maxY = this.bounds[z].maxY - (this.bounds[z].maxY % this.metatile) + this.metatile - 1;
            this.stats.total += (maxX - minX + 1) *
                                (maxY - minY + 1);
        }
    }
...
ScanlineScheme.prototype.nextMetatile = function() {
    ...
    var minX = pos.x - (pos.x % this.metatile);
    var maxX = Math.min(minX + this.metatile, Math.pow(2, pos.z));
    var minY = pos.y - (pos.y % this.metatile);
    var maxY = Math.min(minY + this.metatile, Math.pow(2, pos.z));
